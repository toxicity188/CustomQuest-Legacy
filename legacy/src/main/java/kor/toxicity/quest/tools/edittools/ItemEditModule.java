...
public class ItemEditModule extends EditTools {
	...
	public void load(Player player, String type, String file, String key) {
		...
		if (!sort) inv.setItem(center(), InvUtil.createItem(Material.ENCHANTED_BOOK, prefix + "§fCenter", null));
		inv.setItem((guinum -1)*9, InvUtil.createItem(Material.STONE_BUTTON, prefix + "§fBack without Save", null));
		inv.setItem(guinum * 9 - 1, InvUtil.createItem(Material.STONE_BUTTON, prefix + "§fSave and Back", null));
	...
	public void InvClick(InventoryClickEvent e) {
		if (e.getWhoClicked().equals(player) && e.getInventory().equals(inv)) {
			...
			if (e.getSlot() == center() && !sort && e.getCurrentItem() != null && e.getCurrentItem().getItemMeta().getDisplayName().equals(prefix + "§fCenter")) {e.setCancelled(true); return;}
